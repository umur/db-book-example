...
--     Requires wal_level = logical; skipped with a NOTICE otherwise.
DO $$
BEGIN
    PERFORM pg_create_logical_replication_slot('cdc_movies', 'test_decoding');
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'pg_create_logical_replication_slot skipped: % '
                     '(set wal_level=logical to enable this demo)', SQLERRM;
END$$;
...
--      Returns empty if the slot was not created (wal_level != logical).
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = 'cdc_movies') THEN
        PERFORM lsn FROM pg_logical_slot_peek_changes('cdc_movies', NULL, 10);
        RAISE NOTICE 'Logical slot peek completed (see psql output for decoded rows)';
    ELSE
        RAISE NOTICE 'Skipping logical slot peek: cdc_movies slot not available';
    END IF;
END$$;
...
FROM pg_replication_slots
WHERE slot_name IN ('orphan_demo', 'cdc_movies');
