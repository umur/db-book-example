...
--     Run the LISTEN below interactively. Then in a SECOND psql session,
--     INSERT a new review using the section (1) pattern. The triggers fire
--     pg_notify on commit, and this session prints the notifications
--     immediately. LISTEN blocks non-interactive sessions, so it is
--     commented out here; uncomment when running in a live psql terminal.
