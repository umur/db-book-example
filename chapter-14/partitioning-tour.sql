...
--      same screen. Both are run inside a transaction so the savepoint
--      can catch the expected error without aborting the session.
BEGIN;
...
-- Savepoint lets the session continue after the expected rejection.
...
RELEASE SAVEPOINT before_overlap;
