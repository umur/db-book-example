...
--
-- Bring up the two-container sandbox first:
--   docker compose up -d
...
--
-- When running against a single container (probe/CI), the subscription
-- steps that need a live publisher are intentionally skipped.
--
...
--
-- When running against a single container (probe/CI without docker compose),
-- this step is expected to fail with a connection error and is silently
-- skipped by the SqlRunner error handler.
-- ---------------------------------------------------------------------------
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'CREATE SUBSCRIPTION skipped (publisher not reachable): %', SQLERRM;
END$$;
